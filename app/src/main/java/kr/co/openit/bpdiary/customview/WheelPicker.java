...
public abstract class WheelPicker extends View {
    ...
    public void setOnResizeListener(WheelPickerOnResizeListener orlExt)
    {
        wpResizeListener = orlExt;
    }

    WheelPickerOnResizeListener wpResizeListener = null;
    ...
    public WheelPicker(Context context, AttributeSet attrs) {
        ...
        mSelectedItemTextColor = a.getColor(R.styleable.WheelPicker_wheel_selected_item_text_color,
                                            getResources().getColor(R.color.color_2e2e2e));
        mItemTextColor = a.getColor(R.styleable.WheelPicker_wheel_item_text_color,
        ...
        minValue = a.getInt(R.styleable.WheelPicker_wheel_min_value, 0);

        maxValue = a.getInt(R.styleable.WheelPicker_wheel_max_value, 255);
    ...
    protected void onSizeChanged(int w, int h, int oldW, int oldH) {
        ...
        if(wpResizeListener != null)
        {
            wpResizeListener.onResize(this.getId(), w, h, oldW, oldH);
        }


    }

    public void computeDrawnCenter() {
        switch (mItemAlign) {
            case ALIGN_LEFT:
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int drawnDataPos = drawnDataStartPos
            ...
            paint.setTextSize(PhoneUtil.convertDpToPixel(24));
...
//                paint.setTextSize(PhoneUtil.convertDpToPixel(28));
...
//                }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (isClick && scroller.isFinished() && !isForceFinishScroll) { // WheelPicker clicked
                    if (onWheelClickListener != null) {
                        onWheelClickListener.onWheelSelected();
                        break;
                    }
    ...
    public void scrollTo(final int itemPosition) {
        ...
        if (itemPosition != currentItemPosition) {
            final int differencesLines = currentItemPosition - itemPosition + 1;
            ...
            va.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    currentItemPosition = itemPosition - 1;
                    onItemSelected();
                }
            });
            va.start();
...
//        computeFlingLimitY();
    ...
    public void setOnWheelClickListener(OnWheelClickListener listener) {
        onWheelClickListener = listener;
    }
