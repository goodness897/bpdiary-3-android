...
import kr.co.openit.bpdiary.interfaces.IRangeSeekBar;
...
public class RangeSeekBar<T extends Number> extends ImageView {
    ...
    public static final Integer DEFAULT_GRADIENT_COUNT = 3;

    public static final int HEIGHT_IN_DP = 76;
    ...
    private static final int DEFAULT_TEXT_DISTANCE_TO_BUTTON_IN_DP = 12;

    private static final int DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP = 16;
    ...
    private int leftThumbMax;

    private int rightThumbMin;
    ...
    private IRangeSeekBar iRangeSeekBar;

    private int[] colors = new int[DEFAULT_GRADIENT_COUNT];
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        //        int activeColor = R.drawable.bp_combined_shape;
        int thumbShadowColor;
        int defaultShadowColor = Color.argb(75, 0, 0, 0);
        int defaultShadowYOffset = PhoneUtil.dpToPx(context, 2);
        int defaultShadowXOffset = PhoneUtil.dpToPx(context, 0);
        int defaultShadowBlur = PhoneUtil.dpToPx(context, 2);

        if (attrs == null) {
            setRangeToDefaultValues();
            internalPad = PhoneUtil.dpToPx(context, INITIAL_PADDING_IN_DP);
            barHeight = PhoneUtil.dpToPx(context, LINE_HEIGHT_IN_DP);
            ...
            try {
                setRangeValues(extractNumericValueFromAttributes(a,
                ...
                //Gradient 설정
                showGradient = a.getBoolean(R.styleable.RangeSeekBar_showGradient, false);
                gradientCount = a.getInteger(R.styleable.RangeSeekBar_gradientCount, 1);
                minGap = a.getInteger(R.styleable.RangeSeekBar_minGap, 0);
                leftThumbMax = a.getInteger(R.styleable.RangeSeekBar_leftThumbMax, 0);
                rightThumbMin = a.getInteger(R.styleable.RangeSeekBar_rightThumbMin, 0);
                firstColor = a.getColor(R.styleable.RangeSeekBar_firstGradientColor, Color.YELLOW);
                secondColor = a.getColor(R.styleable.RangeSeekBar_secondGradientColor, Color.YELLOW);
                thirdColor = a.getColor(R.styleable.RangeSeekBar_thirdGadientColor, Color.YELLOW);
                fourthColor = a.getColor(R.styleable.RangeSeekBar_fourthGradientColor, Color.YELLOW);
                fifthColor = a.getColor(R.styleable.RangeSeekBar_fifthGradientColor, Color.YELLOW);
                sixthColor = a.getColor(R.styleable.RangeSeekBar_sixthGradientColor, Color.YELLOW);

                if (showGradient) {
                    setGradientColor(gradientCount);
                }
                ...
                if (normalDrawable != null) {
                    thumbImage = PhoneUtil.drawableToBitmap(normalDrawable);
                }
                Drawable disabledDrawable = a.getDrawable(R.styleable.RangeSeekBar_thumbDisabled);
                if (disabledDrawable != null) {
                    thumbDisabledImage = PhoneUtil.drawableToBitmap(disabledDrawable);
                }
                Drawable pressedDrawable = a.getDrawable(R.styleable.RangeSeekBar_thumbPressed);
                if (pressedDrawable != null) {
                    thumbPressedImage = PhoneUtil.drawableToBitmap(pressedDrawable);
                }
                ...
                thumbShadowXOffset = a.getDimensionPixelSize(R.styleable.RangeSeekBar_thumbShadowXOffset,
                                                             defaultShadowXOffset);
                thumbShadowYOffset = a.getDimensionPixelSize(R.styleable.RangeSeekBar_thumbShadowYOffset,
        ...
        //        if(activeImage == null) {
        //            activeImage = BitmapFactory.decodeResource(getResources(), activeColor);
        //        }
        ...
        thumbHalfWidth = 0.5f * thumbPressedImage.getWidth();
        thumbHalfHeight = 0.5f * thumbPressedImage.getHeight();

        //        activeHalfWidth = 0.5f * activeImage.getWidth();
        //        activeHalfHeight = 0.5f * activeImage.getHeight();

        setValuePrimAndNumberType();

        textSize = PhoneUtil.dpToPx(context, DEFAULT_TEXT_SIZE_IN_DP);
        distanceToTop = PhoneUtil.dpToPx(context, DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP);
        textOffset = !showTextAboveThumbs ? 0
                                          : this.textSize
                                            + PhoneUtil.dpToPx(context, DEFAULT_TEXT_DISTANCE_TO_BUTTON_IN_DP)
                                            + this.distanceToTop;
        ...
        if (showGradient) {
            shader = new LinearGradient(padding / 2,
                                        (getHeight() - (thumbPressedImage.getHeight() / 2)) - barHeight / 2,
                                        getWidth() - (padding / 2),
                                        (getHeight() - (thumbPressedImage.getHeight() / 2)) + barHeight / 2,
                                        colors,
                                        null,
    ...
    public void setGradientColor(int gradientCount) {
        switch (gradientCount) {
            case 1:
                colors = new int[] {firstColor};
                break;
            case 2:
                colors = new int[] {firstColor, secondColor};
                break;
            case 3:
                colors = new int[] {firstColor, secondColor, thirdColor};
                break;
            case 4:
                colors = new int[] {firstColor, secondColor, thirdColor, fourthColor};

                break;
            case 5:
                colors = new int[] {firstColor, secondColor, thirdColor, fourthColor, fifthColor};

                break;
            case 6:
                colors = new int[] {firstColor, secondColor, thirdColor, fourthColor, fifthColor, sixthColor};
                break;
     ...
     *
     * @param value The Number value to set the minimum value to. Will be clamped to given absolute minimum/maximum
     * range.
     */
     ...
     *
     * @param value The Number value to set the maximum value to. Will be clamped to given absolute minimum/maximum
     * range.
     */
    ...
    /**
     * Set the path that defines the shadow of the thumb. This path should be defined assuming that the center of the
     * shadow is at the top left corner (0,0) of the canvas. The {@link #drawThumbShadow(float, Canvas)} method will
     * place the shadow appropriately.
     *
    ...
    private void trackTouchEvent(MotionEvent event) {
        ...
        if (Thumb.MIN.equals(pressedThumb) && !singleThumb) {
            ...
            Log.d("Touch", "min : " + screenToNormalized(x));
        } else if (Thumb.MAX.equals(pressedThumb)) {
            setNormalizedMaxValue(screenToNormalized(x));
            Log.d("Touch", "max : " + screenToNormalized(x));
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = thumbPressedImage.getHeight()
                     + (!showTextAboveThumbs ? 0 : PhoneUtil.dpToPx(getContext(), HEIGHT_IN_DP))
    ...
    protected synchronized void onDraw(@NonNull Canvas canvas) {
        ...
        if (showLabels) {
            ...
            paint.setColor(getResources().getColor(R.color.color_a5a5a5));
            paint.setTextSize(PhoneUtil.dpToPx(getContext(), 12));
            canvas.drawText(minLabel, minMaxLabelSize, minMaxHeight + (minMaxHeight / 2), paint);
            canvas.drawText(maxLabel,
                            getWidth() - (minMaxLabelSize + (PhoneUtil.dpToPx(getContext(), 11) * 2)),
                            minMaxHeight + (minMaxHeight / 2),
                            paint);
        }
        ...
        if (showGradient) {
            paint.setShader(new LinearGradient(padding / 2,
                                               (getHeight() - (thumbPressedImage.getHeight() / 2)) - barHeight / 2,
                                               getWidth() - (padding / 2),
                                               (getHeight() - (thumbPressedImage.getHeight() / 2)) + barHeight / 2,
                                               colors,
        ...
        if (!alwaysActive && !activateOnDefaultValues && selectedValuesAreDefault) {
            ...
            // Seek
            rectF.left = padding / 2;
            rectF.right = normalizedToScreen(normalizedMinValue);
            canvas.drawRect(rectF, paint);

            paint.setShader(null);
            paint.setColor(defaultColor);
            rectF.left = normalizedToScreen(normalizedMaxValue);
            rectF.right = getWidth() - (padding / 2);
            canvas.drawRect(rectF, paint);
            //            paint.setColor(activeColor);
        }
        ...
        float maxPosition = Math.min(getWidth() - maxTextWidth,
                                     normalizedToScreen(normalizedMaxValue) - maxTextWidth * 0.5f);

        if (iRangeSeekBar != null) {
            iRangeSeekBar.min(minText);
            iRangeSeekBar.max(maxText);
        }
        if (!singleThumb && showLabels) {
            // check if the labels overlap, or are too close to each other
            int spacing = PhoneUtil.dpToPx(getContext(), TEXT_LATERAL_PADDING_IN_DP);
    ...
    private void drawLine(Canvas canvas) {
        float minLinePosition = Math.max(0f, normalizedToScreen(normalizedMinValue));
        float maxLinePosition = Math.min(getWidth(), normalizedToScreen(normalizedMaxValue));
        canvas.drawRect((minLinePosition - barHeight / 6),
                        0,
                        (minLinePosition + barHeight / 6),
                        getHeight() - (thumbPressedImage.getHeight() / 2),
                        paint);
        canvas.drawRect((maxLinePosition - barHeight / 6),
                        0,
                        (maxLinePosition + barHeight / 6),
                        getHeight() - (thumbPressedImage.getHeight() / 2),
                        paint);

        Paint rangPaint = new Paint();
        rangPaint.setColor(getResources().getColor(R.color.color_33ffffff));
        canvas.drawRect((minLinePosition - barHeight / 6),
                        barHeight,
                        (maxLinePosition + barHeight / 6),
                        getHeight() - (thumbPressedImage.getHeight() / 2),
        ...
        canvas.drawCircle((minLinePosition), barHeight, barHeight, paint);
        canvas.drawCircle((maxLinePosition), barHeight, barHeight, paint);
    }
    ...
    private void drawThumb(float screenCoord, boolean pressed, Canvas canvas, boolean areSelectedValuesDefault) {
        if (!pressed) {
            int x = getHeight() - thumbImage.getHeight() - (thumbPressedImage.getHeight() - thumbImage.getHeight()) / 2;
            canvas.drawBitmap(thumbDisabledImage,
                              screenCoord - ((thumbHalfWidth)
                                             - (thumbPressedImage.getWidth() - thumbImage.getWidth()) / 2),
                              x,
                              paint);
        } else {
            int x = getHeight() - thumbPressedImage.getHeight();
            canvas.drawBitmap(thumbPressedImage, screenCoord - thumbHalfWidth, x, paint);
        }
        //        if (!pressed) {
        //            canvas.drawBitmap(thumbDisabledImage, screenCoord - ((thumbHalfWidth) - (thumbPressedImage.getWidth() - thumbImage.getWidth()) / 2),
        //                    ((textOffset) + (thumbPressedImage.getHeight() - thumbImage.getHeight()) / 2),
        //                    paint);
        //        } else {
        //            canvas.drawBitmap(thumbPressedImage, screenCoord - thumbHalfWidth,
        //                    textOffset,
        //                    paint);
        //        }
    }
    ...
    private void drawThumbShadow(float screenCoord, Canvas canvas) {
        thumbShadowMatrix.setTranslate(screenCoord + thumbShadowXOffset,
                                       textOffset + thumbHalfHeight + thumbShadowYOffset);
    ...
    private void setNormalizedMinValue(double value) {
        double x = 0;
        if (minGap != 0) {
            x = minGap / (absoluteMaxValuePrim - absoluteMinValuePrim);
            normalizedMinValue = Math.max(0d, Math.min(1d, Math.min(value, normalizedMaxValue - x)));
        } else if (leftThumbMax != 0) {
            x = (leftThumbMax - absoluteMinValuePrim) / (absoluteMaxValuePrim - absoluteMinValuePrim);
            normalizedMinValue = Math.max(0d, Math.min(1d, Math.min(value, x)));
        } else {
    ...
    private void setNormalizedMaxValue(double value) {
        ...
        if (minGap != 0) {
            x = minGap / (absoluteMaxValuePrim - absoluteMinValuePrim);
            normalizedMaxValue = Math.max(0d, Math.min(1d, Math.max(value, normalizedMinValue + x)));
        } else if (rightThumbMin != 0) {
            x = (rightThumbMin - absoluteMinValuePrim) / (absoluteMaxValuePrim - absoluteMinValuePrim);
            normalizedMaxValue = Math.max(0d, Math.min(1d, Math.max(value, x)));
        } else {
    ...
    protected T normalizedToValue(double normalized) {
        ...
        return (T)numberType.toNumber(Math.round(v * 10) / 10d);
    }
    ...
    public void setIRangeSeekBar(IRangeSeekBar iRangeSeekBar) {
        this.iRangeSeekBar = iRangeSeekBar;
    }
    ...
    public void setBarDatas(int[] datas, int yMax) {
        this.datas = datas;
        this.ymax = yMax;
        invalidate();
    }
    ...
    private void drawBar(Canvas canvas, Paint paint) {
        if (datas != null && ymax != 0) {
            float y = 0;
            float beforOneXPoint = padding;
            float bottom = (getHeight() - (thumbPressedImage.getHeight() / 2)) - barHeight / 2;
            float oneXPoint = ((getWidth() - (padding * 2)) / datas.length);
            float oneYPoint = bottom / ymax;
            for (int i = 1; i < datas.length + 1; i++) {
                if (datas[i - 1] == 0) {
                    y = bottom;
                } else {
                    y = bottom - (oneYPoint * datas[i - 1]) + barHeight;
                }
                float x1 = beforOneXPoint;
                float x2 = x1 + oneXPoint;
                canvas.drawRect(x1, y, x2, bottom, paint);
                beforOneXPoint = x2;
            }
        }
    }
